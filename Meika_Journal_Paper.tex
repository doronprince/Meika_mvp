...
\begin{abstract}
Personal financial management applications are abundant, yet most remain retrospective: they record transactions and render historical summaries without projecting a user's future financial position. Where artificial intelligence is applied, its outputs usually reach the user as bare predictions or scores, with no account of the reasoning that produced them. Recent surveys identify this transparency deficit as a principal obstacle to trust in financial AI. This paper presents MEIKA, an Explainable AI (XAI)-based personal financial copilot in which explainability is a structural constraint on every output. MEIKA computes two predictive budgeting indicators from live transaction data, spending velocity and a projected month-end spend, and combines them with a category-concentration measure into a deterministic Financial Clarity Score. Every component of that score is emitted as a named \texttt{XAIFactor} object carrying a label, an explanatory detail, and the computed value, so no figure reaches the user without its justification. The system is a decoupled monorepo: a Flutter client, a layered FastAPI backend with Pydantic validation and asynchronous SQLAlchemy over a tenant-isolated PostgreSQL schema, and a bounded Gemini reasoning layer restricted to rephrasing already-computed factors. At the Phase-I review the prototype demonstrated three tested REST modules producing a Clarity Score of 91/100 with three live factors over a seeded dataset. The implementation has since been extended with a grounded conversational copilot, a price-comparison module, authentication, and multi-currency display. Explainable goal forecasting and empirical calibration of the score remain the principal items of future work.
\end{abstract}
...
Managing personal finances is one of the most universal, yet persistently difficult, aspects of everyday life. Individuals today manage income and expenditure across a growing number of channels: bank accounts, digital wallets, credit cards, and subscription services. Together these generate a volume of transactional data that few tools help them interpret \cite{badiger2026spendai}. Numerous budgeting and expense-tracking applications have emerged to address this, and empirical reviews show they do what they were built to do, which is to record what has already happened. Existing systems are repeatedly found to suffer from fragmented financial data, limited real-time insight, and tracking methods that stop short of proactive decision-making \cite{deepthi2026aipowered}--\cite{ganta2025multimodule}. Even where machine learning has improved the accuracy of automatic transaction categorization, the decision logic exposed to the user typically remains a bare forecast with nothing attached to explain it \cite{deepthi2026aipowered}.

Advances in Artificial Intelligence (AI), Machine Learning (ML), Explainable AI (XAI), and Large Language Models (LLMs) create an opportunity to turn these tools into genuine financial assistants instead of passive ledgers. Recent prototypes illustrate both the promise and the limits of that shift. Deepthi et al. describe a serverless AI-Powered Finance Management Platform that layers tree-based classifiers, time-series forecasting, and an LLM-based conversational assistant onto a microservices architecture, reporting strong fraud-detection accuracy; the paper itself treats explainability as a secondary addition that the architecture does not require \cite{deepthi2026aipowered}. Ganta et al. propose a multi-module financial assistant integrating fraud detection, credit recommendation, and a document-aware chatbot, and find that existing fintech tools largely operate as isolated modules with little shared context \cite{ganta2025multimodule}. At the model layer, independent surveys of large language models in finance reach a common conclusion. LLMs are increasingly capable of financial reasoning and decision support, while data privacy, bias, and explainability remain the field's most pressing open challenges \cite{nie2024survey}, \cite{khan2025bridging}.

These observations motivate the central premise of the project. The objective is to make transparency a first-class design constraint: every ``buy,'' ``wait,'' or budget-adjustment recommendation should be traceable to the computed factors that generated it, such as spending velocity, category drift, and projected cash-flow risk, instead of resting on an opaque model output the user is asked to trust on faith.

This paper proposes MEIKA, an Explainable AI-Based Personal Financial Copilot for predictive budgeting, financial-goal forecasting, and financial-risk analysis. It is intended for students, salaried professionals, freelancers, and families who want to act on their spending patterns with some confidence about what follows from them.
...
\item A predictive budgeting mechanism based on spending velocity and month-end projection computed from live transaction data, including an explicit warm-up guard that withholds a projection when the underlying data is too thin to support one.
...
Unde et al. similarly propose an AI-Based Real-Time Personal Finance Dashboard that consolidates fragmented financial data through banking-API ingestion, CNN/YOLOv4-based OCR for receipt digitization, BERT/LSTM models for categorization and savings forecasting, and autoencoder-based anomaly detection, paired with an LLM-driven advisory chatbot \cite{unde2026dashboard}. The authors' own review concludes that most contemporary tools address isolated tasks such as receipt scanning or budgeting, with a persistent lack of a unified, real-time dashboard that integrates visual recognition, anomaly detection, and cross-platform data consolidation. Saini et al. take a broader view, surveying AI's role across categorization, budgeting, robo-advisory, and financial-literacy tools (e.g., Mint, Cleo, YNAB, Betterment). They conclude that despite rapid capability growth, data quality, legacy-system integration, algorithmic fairness, and again transparency in AI-generated recommendations remain open, field-wide challenges \cite{saini2025aipowered}.
...
Khan, Li, and Cao similarly survey LLMs across the broader financial system, presenting a taxonomy spanning text processing, forecasting, and question-answering tasks and proposing a multi-level framework for balancing model accuracy, computational cost, and data privacy \cite{khan2025bridging}. The survey identifies data privacy, bias, and explainability as the primary deployment obstacles to trustworthy financial AI, and calls explicitly for ``more transparent, effective, and inclusive financial AI'' as a future research direction. Read together, the two surveys support the design premise of the present work: a financial copilot should expose the reasoning behind its outputs, because trust in such a system is limited by what the user can inspect.

\subsection{Decision Support Systems Foundations}

Beyond finance-specific AI literature, this project draws its architectural grounding from Pathak's survey of comparison-shopping-agent-based decision support systems (DSS), adopted as the base paper for this work \cite{pathak2010survey}. Pathak characterizes a comprehensive DSS as requiring four components, namely data, models, interfaces, and user-specific customization, and uses this framework to synthesize contemporary research on web-based shopping agents. Although developed in the context of e-commerce price comparison rather than personal finance, this four-component framework maps directly onto the proposed system's own architecture: a data layer (transaction and account ingestion), a models layer (predictive budgeting, risk, and Financial Clarity Score computation), an interface layer (the client application and conversational UI), and a customization layer (per-user, tenant-scoped personalization). This framework is used in the present work to structure the system architecture and to justify treating explainability and personalization as core architectural components.

\subsection{Research Gap}

Across this body of work, a consistent pattern emerges. Systems that achieve strong predictive or classification performance (Badiger et al.'s 91\% categorization accuracy \cite{badiger2026spendai}; Deepthi et al.'s 98\% fraud-detection accuracy \cite{deepthi2026aipowered}) treat explainability as an auxiliary feature that does not constrain every output. Systems that integrate multiple financial functions into a single assistant \cite{ganta2025multimodule} do not extend that integration to genuine long-term goal forecasting or a formally justified decision-support architecture, and the one surveyed system built explicitly around goal planning \cite{patil2026nexus} defers both time-series forecasting and any factor-level account of the scores it produces. And the LLM-in-finance surveys \cite{nie2024survey}, \cite{khan2025bridging} confirm, at a field level, that explainability and trust remain unresolved even as underlying model capability continues to grow. No reviewed system combines predictive budgeting, financial-goal forecasting, financial-risk analysis, and a conversational interface within a single platform in which every recommendation is required to carry its own computed justification, grounded in an explicit decision-support framework \cite{pathak2010survey}. This is the gap the proposed system, MEIKA, is designed to address.


\section{Problem Statement and Research Gap}

Most of the systems reviewed in Section II are systems of record. A conventional personal-finance application ingests or accepts transactions, normalizes them into categories, and renders the result as a historical summary, so the user learns what has already happened. Where predictive components are present, they are usually confined to short-horizon categorization or expenditure forecasting \cite{kakulapati2026expensetracker}, \cite{badiger2026spendai}, and their predictions reach the user as bare numbers with nothing attached to justify them.

This produces a compound problem. First, financial difficulty is detected only after it has materialized: a user discovers an overspend at the end of the month, when no corrective action remains available. Second, the artificial-intelligence components introduced to address this are themselves opaque, so a user who is warned has no basis on which to evaluate the warning. Saini et al. document this directly, observing that user trust increases as financial AI shifts away from black-boxed operation toward transparent and explanative behaviour \cite{saini2025aipowered}; Nie et al. and Khan et al. reach the same conclusion at the level of the underlying models \cite{nie2024survey}, \cite{khan2025bridging}. Third, the functions a user needs, including tracking, projection, risk detection, goal planning, and purchase decision support, are spread across single-purpose tools that share neither a schema nor a context. Ganta et al. identify this fragmentation as the central weakness of the contemporary fintech sector \cite{ganta2025multimodule}.
...
The guarantee is the demanding part of the problem. An explanation produced by a secondary interpretability technique applied to a subset of model outputs, as in the SHAP layer of Deepthi et al. \cite{deepthi2026aipowered}, is one the architecture permits without requiring. The position taken here is that explainability must be enforced by construction, so a value that cannot be decomposed into named contributing factors is not shown at all. Table~\ref{tab:researchgap} summarizes the specific gaps identified across the surveyed literature and the corresponding design response in the proposed system.
...
MEIKA is an explainable AI-based personal financial copilot: an application that records expenditure, projects its consequences, and justifies each projection it makes. The system is organized as a set of cooperating modules, and each module below is annotated with its verified implementation status.
...
The Clarity Score is a bounded integer in $[0, 100]$ expressing the user's current financial risk posture, accompanied by a categorical risk level. It is computed deterministically by subtracting penalties contributed by each analysed factor from a nominal maximum, as formalized in Section VI. No trained model is involved. There is no training data, no set of learned weights, and no inference step, which is what makes the score auditable line by line.
...
Goal forecasting would project whether a defined savings or spending target remains achievable given the observed spend trend. It is a stated objective of the project and is not implemented at the time of writing; a source-level inspection of the backend and client confirms that no goal model, service, or endpoint exists. This paper treats it throughout as a planned extension.

\subsection{Conversational Copilot (Implemented, with a Bounded and Partially Unvalidated Model Path)}

The conversational layer is exposed over a WebSocket endpoint and is grounded by construction: replies are assembled from the same computed Clarity Score factors and price-comparison results that the dashboard uses, and chat history is persisted. Its status has three parts. The transport, grounding pipeline, history persistence, and deterministic reply path are implemented and covered by automated tests. The live Gemini generation path is implemented and wrapped so that any failure falls back to the deterministic path, but it has not been validated against a live API key. The model's role is bounded by design: it may rephrase already-computed figures conversationally, and it cannot introduce a value the deterministic layer did not compute.

\subsection{Price-Finder and True Economic Cost (Implemented)}

A supporting module compares the true economic cost of a prospective purchase across retailers, meaning the sticker price plus the transit cost of acquiring it, and computes its recommendation from those figures. It operates over a seeded catalog and, where an external search key is configured, over live retail search results, falling back to the seeded catalog when the external service is unavailable. A listing observed only once has its price trend reported as \texttt{insufficient\_data}, since one observation cannot establish a direction.
...
The architecture needs no training infrastructure. Because the risk model is deterministic, there is no training corpus, no retraining cycle, and no accuracy figure that can drift as a learned distribution ages. The system gives up the pattern-discovery ability of a learned model in exchange for outputs that are reproducible and auditable.
...
Equation (2) is evaluated only when $d \geq d_{\min}$, with $d_{\min} = 7$ in the current implementation. Below that threshold the projection is withheld and replaced by an explanatory factor stating why.
...
The resulting decomposition is summarized in Fig.~\ref{fig:xaimodel}. Equations (4) through (6) make the score explainable in the sense used here: each penalty is a function of exactly one named, independently observable quantity, so a user can check how much each factor contributed to the final score and dispute it if it looks wrong. Concentration measures how much of the month's expenditure falls in a small number of categories. The implementation penalizes only the share above a $40\%$ tolerance, on the reasoning that some concentration is normal and only pronounced concentration signals fragility.
...
The client renders the score together with its factor list. The Budget screen additionally derives a cash-flow runway on the client from the values returned by the API, dividing the remaining budget by the spending velocity to obtain the number of days the budget will last at the current pace and the calendar date on which it would be exhausted. The runway is client-side arithmetic over server-computed inputs; the server does not model it separately.
...
The \texttt{XAIFactor} type is defined once in the shared schema module and reused by the dashboard, price-finder, and copilot response models, so a single definition of ``explanation'' applies across every surface that produces one. The Clarity Score response cannot be constructed without a factor list, so the type system enforces the explainability constraint instead of leaving it to developer discipline.

\subsection{Security}

Authentication is by signed JSON Web Token, with bcrypt password hashing; the interim header-based user identification used during early development has been removed. An in-process sliding-window rate limiter applies a general per-minute request budget and a stricter budget on authentication routes. As documented in the project repository, this is an MVP security baseline and has not been through a full security review: there is no refresh-token or revocation flow, no brute-force lockout beyond the rate limit, no transport-security enforcement at the application layer, and the client holds its token in memory only. The rate limiter is per-process and would require a shared store before running multiple workers.

\subsection{Testing}

The test suite comprises eleven pytest modules covering health, authentication, users, expenses, dashboard analytics, price comparison, live price search, foreign exchange, and chat. At the Phase-I demonstration, nine of eleven automated tests passed. The suite has since grown substantially. No current pass rate is reported here, because running the complete suite requires a provisioned PostgreSQL instance that was unavailable in the environment used to prepare this manuscript, and a partial run reported as a headline figure would misrepresent the result. External-service tests are constructed so that they never consume live API quota, using a fixture that disables the external search key and a mocked HTTP client to verify parsing logic.
...
This section reports a functional and architectural evaluation of a working prototype, and makes no claim about predictive accuracy. The system contains no trained model, so accuracy, precision, recall, and error metrics such as RMSE or MAPE are undefined for the components currently implemented. Reporting them now would be meaningless, and not merely premature.
...
The property that distinguishes the system is structural, so this part of the evaluation is structural too. In a conventional pipeline a model produces the score, and any explanation is reconstructed afterwards by a separate interpretability technique applied to that model. That leaves two failure modes: the explanation may be missing for some outputs, and where present it approximates the model's reasoning. In MEIKA the penalty and the factor describing it are built in the same operation from the same quantities, so the explanation states the computation exactly. The demonstrated output shows what this means for a user. Alongside the score of 91, the user can see that spending was running at 112\% of the even-pace budget, that the month-end projection exceeded the budget, and that the largest category stayed below the concentration tolerance.

The guarantee covers the values the deterministic layer computes. It does not cover the language model's phrasing of those values, which prompt construction and the deterministic fallback path constrain but which has not been formally verified.
...
Two qualifications apply. A seven-transaction dataset supports no statistical inference; these figures show that the pipeline computes and explains correctly, and they say nothing about whether the scoring parameters are calibrated against real financial behaviour. The 27 August runway is client-side arithmetic dividing the remaining budget by the server-computed velocity, as described in Section VI, and it has all the weaknesses of a linear extrapolation from a small sample.

\subsection{Worked Verification of the Scoring Equations}

Because the scoring pipeline is deterministic, the demonstrated output can be recomputed by hand from the equations of Section VI. Doing so checks that the formalization in this paper matches the running implementation. The demonstration was taken on day $d = 13$ of a $D = 31$ day month, against a monthly budget of $B = 600{,}000$~KRW, with month-to-date expenditure of $S = 282{,}850$~KRW distributed such that the largest category accounted for $90{,}350$~KRW.
...
The penalties then follow. From (4), $\Pi_{\text{pace}} = 0.124 \times 35 = 4.34$. From (5), $\Pi_{\text{proj}} = (74{,}488/600{,}000) \times 40 = 4.97$. The largest category represents $90{,}350/282{,}850 = 31.9\%$ of expenditure, which falls below the $40\%$ tolerance, so from (6) $\Pi_{\text{conc}} = 0$. Substituting into (7) gives $\mathrm{CS} = \lfloor 100 - 9.31 \rceil = 91$, classified as low risk since $\mathrm{CS} \geq 70$. This is the score the system displayed. The client-side runway of Section VI likewise reproduces: $317{,}150 / 21{,}758 = 14.6$, floored to 14 days from 13 August, yielding the 27 August exhaustion date shown.

Any user, auditor, or reviewer can recompute a Clarity Score this way from the factors displayed alongside it, and no access to a model is needed because the score does not come from one.

\subsection{Implementation Status at the Time of Writing}

The prototype has advanced considerably since the Phase-I review, as recorded in Table~\ref{tab:status}. Three modules listed as pending at the review have since been implemented: the copilot WebSocket integration, the copilot chat interface, and the authentication and rate-limiting layer. The price-comparison module, manual expense entry, and multi-currency display have also been added. Goal forecasting remains unimplemented and is the principal outstanding objective.
...
The evaluation above establishes functional correctness but not calibration. A fuller evaluation is planned once a larger dataset and a user cohort are available; the intended measures are listed in Table~\ref{tab:futuremetrics}. No results are reported against these measures yet.
...
Table~\ref{tab:comparison} positions MEIKA against two reference classes drawn from the reviewed literature: the conventional expense tracker, which records and reports but does not predict, and the black-box AI finance system, which predicts capably but exposes little of its reasoning. The table describes design choices and makes no claim of measured superiority. The systems in the second column report predictive accuracies that MEIKA has not tried to match, and without a trained model it does not compete with them on that measure.
...
The last two rows count against the proposed system. A deterministic scoring engine cannot discover a spending pattern nobody anticipated, and a learned model can. MEIKA accepts that limitation in exchange for something a learned model cannot offer: every figure shown can be decomposed, reproduced, and contested. The claim is limited to outputs a user is expected to act on financially. For those, this paper argues, an auditable computation is preferable to an unauditable one of comparable or somewhat greater accuracy. Where pattern discovery is genuinely needed, the extension proposed is a learned component whose outputs are themselves decomposed into named factors before display.

\section{Limitations}

The claims above are subject to the following limitations.
...
\emph{Security is an unaudited MVP baseline.} Authentication and rate limiting are implemented, but there is no token revocation or refresh flow, no brute-force lockout beyond the rate limit, no application-layer transport-security enforcement, and no secure client-side token storage. The rate limiter holds state per process and would not function correctly across multiple workers.

\emph{No user study.} No evaluation of whether the factor-based presentation actually improves user comprehension or trust has been conducted. The explainability claim in this paper is structural: the explanation faithfully describes the computation. That does not by itself show that users find the explanation useful, and this is the largest gap between what the architecture guarantees and what the project aims to demonstrate.

\emph{Price-trend reporting is constrained by observation history.} Listings retrieved from live external search are observed once and therefore carry no trend, which the system reports as insufficient data instead of inferring a direction.

\section{Future Work}

The following directions address the limitations above.
...
\item \textbf{User studies on explainability and trust.} Compare factor-annotated against bare-score presentation on comprehension, trust, and acted-upon recommendations. The structural claim of this paper does not supply this evaluation.
...
\item \textbf{Advanced financial risk modelling.} Introduce additional risk axes such as income volatility, obligation coverage, and liquidity buffer, each admitted only if it can be expressed as a named, auditable factor.
\item \textbf{Bounded learned components.} Where pattern discovery is required, investigate learned models whose outputs are decomposed into named factors before display, so that the explainability constraint still applies.
\end{enumerate}

\section{Conclusion}

Personal financial management applications are abundant, but most remain systems of record: they report what has happened and leave the user to infer what follows. Where artificial intelligence has been introduced to close that gap, it has mostly supplied predictions with little to explain them. The surveys examined in Section II agree on the result: explainability and trust now limit the usefulness of financial AI more than raw model capability does.

This paper has presented MEIKA, an explainable AI-based personal financial copilot designed around explainability as an architectural constraint. The system computes spending velocity and month-end projection directly from live transaction data, combines them with a category-concentration measure into a deterministic Financial Clarity Score, and emits every contributing factor as a named \texttt{XAIFactor} object in the same response as the score. The penalty and its explanation are built in one operation from the same quantities, so the explanation states the computation exactly, and a score cannot be displayed without its reasoning. The system is realized as a Flutter client over a layered FastAPI backend and a tenant-isolated PostgreSQL schema, with a language-model layer bounded to rephrasing figures the deterministic layer has already computed.

The contribution is limited in scope. At the Phase-I review the prototype demonstrated three tested REST modules producing a Clarity Score of 91 out of 100 with three live factors over a seven-transaction dataset, and the implementation has since gained a grounded conversational copilot, a price-comparison module, authentication, and multi-currency display. This shows the approach can be built. It does not show the approach is calibrated: the scoring parameters remain empirically unvalidated, goal forecasting is not yet implemented, and no user study has tested whether users find the guaranteed explanations useful. The next phase of the work addresses those questions. What this paper establishes is that a financial decision-support system can be built so that no figure reaches the user without its justification, and it argues that this property should be designed in from the start.
